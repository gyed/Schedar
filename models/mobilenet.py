...
__all__ = ['MobileNet_L', 'MobileNet_M', 'MobileNet_S']

class MobileNet_L(nn.Module):
    def __init__(self, in_channels, output_size=10, alpha = 1):
        super(MobileNet_L, self).__init__()
        self.output_size = output_size

        # first conv layer 
        self.conv0 = nn.Conv2d(in_channels, int(32*alpha), kernel_size = 3, stride = 1, padding = 1, bias = False)
...
class MobileNet_M(nn.Module):
    def __init__(self, in_channels, output_size=10, alpha = 1):
        super(MobileNet_M, self).__init__()
        self.output_size = output_size
        ...
        self.bottlenecks = nn.Sequential(
            ...
            BaseBlock(16, 24, downsample = False),
            BaseBlock(24, 32, downsample = False),
            BaseBlock(32, 48, downsample = True),
            BaseBlock(48, 64, downsample = False),
            BaseBlock(64, 96, downsample = True),
            BaseBlock(96, 160, downsample = False)
        )

        # last conv layers and fc layer
        self.conv1 = nn.Conv2d(int(160*alpha), 320, kernel_size = 1, bias = False)
...
class MobileNet_S(nn.Module):
    def __init__(self, in_channels, output_size=10, alpha = 1):
        ...
        BaseBlock.alpha = alpha
        self.bottlenecks = nn.Sequential(
            BaseBlock(32, 8, t = 1, downsample = False),
            BaseBlock(8, 8, downsample = True),
            BaseBlock(8, 8, downsample = True),
        )

        # last conv layers and fc layer
        self.conv1 = nn.Conv2d(int(8*alpha), 8, kernel_size = 1, bias = False)
